...
void g4e_read()
{
  ...
  auto el_e_tot_roman = new TH1D("el_e_tot_roman", "el_e_tot_roman", 150, 0., 15.);
  ...
  while (fReader.Next())
    {
      if(++events_numer > 2000) break; // The next operation may take time so we want to limit NEvents
      ...
      for(size_t i = 0 ; i < hits_count ; i++)
	{
	  ...
	  if( track_ids_in_ecap_emcal.count(trk_id[i]) )
	    {
	      //continue;
	      cout << "track in the ce_EMCAL" << endl;
	  ...
	  if( track_ids_in_ffi_RPOTS.count(trk_id[i]) )
	    {
	      //continue;
	      cout << "track in the roman pot" << endl;
	      // Construct TLorenz vector
	      px = trk_vtx_dir_x[i] * trk_mom[i];
	      py = trk_vtx_dir_y[i] * trk_mom[i];
	      pz = trk_vtx_dir_z[i] * trk_mom[i];
